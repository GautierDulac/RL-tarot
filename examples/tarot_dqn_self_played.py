...
if not os.path.exists('rlcard/models'):
    ...
    if not os.path.exists('rlcard/models/pretrained'):
        ...
        for self_play in range(1, total_self_play_eval + 1):
            if not os.path.exists(
                    'rlcard/models/pretrained/tarot_v' + str(record_number * total_self_play_eval + self_play)):
                os.makedirs('rlcard/models/pretrained/tarot_v' + str(record_number * total_self_play_eval + self_play))
model_path = 'rlcard/models/pretrained/tarot_v' + str(record_number * total_self_play_eval + self_play) + '/model'
...
with tf.compat.v1.Session() as sess:
    ...
    for episode in range(episode_num):
        ...
        if episode % evaluate_every == 0:
            # Save Model
            model_path = 'rlcard/models/pretrained/tarot_v' + str(
                record_number * total_self_play_eval + self_play) + '/model'
